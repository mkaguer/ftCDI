...
tf = 1000
Ca = 48
filename = "../data/cdi_simulation_unperforated_" + str(tf) + "s_" + str(Ca)
...
# get c_out for discharge curve assuming CSTR
t_mix = 60 # s
...
def fun(x, y, t_mix):
    ...
    c = np.interp(x, tp, cp)
    return (c - y)/t_mix
soln = solve_ivp(fun, (t[0], t[-1]), y0=c_out[0:1], method="BDF",
                 t_eval=t, args=(t_mix,))
c_out = soln.y[0, :]

# load experiment data
data_exp = np.loadtxt("../figures/guyes2017b/discharge-unperforated-cycle1.csv",
...
ts = t + 160
c_out_exp = np.interp(ts, t_exp, c_out_exp)

# discharge curve
plt.figure(1)
plt.plot(ts, c_out, color="orange", linewidth=3, label=f"PNM (Ca={Ca}F/mL)")
plt.plot(ts, c_out_exp, color="k", linewidth=3, label="Experiment")
...
# load current data
data_exp = np.loadtxt("../figures/guyes2017b/current-unperforated-cycle1.csv",
                      delimiter=",", skiprows=1)
t_exp = data_exp[:, 0]
I_exp = data_exp[:, 1]
