...
This script works with some extras commented out! Must use mu_att = 2.5

Works but did not scale well! Needed to change how update A and b is handled!
Use _FINAL for version that can scale.
...
mu_att = 2.5  # prpts.properties["mu_att"]
...
def run_charge(network, bc1, bc2):
    ...
    pnm.algorithms.set_source(proj, alg="alg4",
                              pores=net["pore.micropore"],
                              propname="charge_source")
    
    # build A and b
    ct["A"] = pnm.algorithms.build_A(proj, "alg4")
    ct["b"] = pnm.algorithms.build_b(proj, "alg4")
    
    # set BCs, set phi @ separator to zero helped stabilize gmres
    pores = net["pore.separator"] + net["pore.perforated"]
    pnm.algorithms.set_BC(proj, "alg4", pores, bctype='value',
                          bcvalues=0, mode="overwrite")
    
    # set BCs at micropores
    pores = net["pore.micropore"]*net["pore.anode"]
    pnm.algorithms.set_BC(proj, "alg4", pores, bctype='value',
                          bcvalues=bc1, mode="add")
    pores = net["pore.micropore"]*net["pore.cathode"]
    pnm.algorithms.set_BC(proj, "alg4", pores, bctype='value',
                          bcvalues=bc2, mode="add")
    
    # apply sources
    ct["A"], ct["b"] = pnm.algorithms.apply_sources(proj, alg="alg4")
    
    # apply BCs
    ct["A"], ct["b"] = pnm.algorithms.apply_BC(proj, alg="alg4")
    
    # solve
    x = pnm.algorithms.solve(proj, "alg4")
...
def _update_mass_source(network, c, phi):
    ...
    print(jnp.sum(jnp.abs(network["pore.mass_source"]["rate"])))
...
def charge_solve(network,
    ...
    def body(state):
        ...
        # solve for phi
        phi_new, _ = jax.scipy.sparse.linalg.bicgstab(
            matvec,
            bc_t,
            x0=phi,
            tol=tol,
...
def mass_solve(network,
    ...
    def body(state):
        ...
        # solve for c
        c_new, _ = jax.scipy.sparse.linalg.bicgstab(
            matvec,
            bm_t,
            x0=c,
            tol=tol,
