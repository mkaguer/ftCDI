...
# TEST: adjust throat diameter of micropores
# net["throat.diameter"] = jnp.where(net["throat.micropore"], 1.4142135623730951e-05, net["throat.diameter"])
...
for t in jnp.arange(t0+dt, tf+0.9*dt, dt):
    ...
    for g_iter in range(g_max_iter):
        ...
        while p < p_max_iter:
            ...
            # update potential
            # print("Residual")
            # print(jnp.sum(jnp.abs(phi - phi_new)))
            phi = a * phi_new + (1 - a) * phi_old
        # update potential on net
        net["pore.potential"] = phi
        p = 0
        p_max_iter = 1
        while p < p_max_iter:
            ...
            c_new = jnp.clip(c_new, 1e-6, jnp.inf)
            # update concentration
            c = w * c_new + (1 - w) * c_old
        # update concentration on net
