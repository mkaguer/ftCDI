...
import collection.numpy as collection
...
# set zeta
net["throat.zeta"] = 1.0
net["throat.zeta@micropore"] = 1.0  # this is the one "trick" I need to make work!
...
start = time.time()
eff_vol = models.numpy.effective_volume.bcc_fast(network=net,
...
for t in np.arange(t0 + dt, tf+dt, dt):
    ...
    for g_iter in range(g_max_iter):
        ...
        c = np.clip(c, 0, np.inf)  # this was necessary when 
...
c_mi = phase["pore.micro_concentration@micropore"]/2 - cf*np.exp(mu_att)
