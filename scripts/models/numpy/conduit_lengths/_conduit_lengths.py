...
def spheres_and_cylinders(
    ...
    L1 = np.where(D1 > Dt, np.sqrt(D1**2 - Dt**2) / 2, 1e-32)
    L2 = np.where(D2 > Dt, np.sqrt(D2**2 - Dt**2) / 2, 1e-32)
    Lt = L_ctc - (L1 + L2)
...
def intersecting_trapezoidals(network):
    ...
    L1 = np.linalg.norm(t_coords - coords[P1], axis=1)
    L2 = L_ctc - L1
    Lt = np.ones_like(L1) * 1e-32
