...
def intersecting_trapezoidals(
    ...
    h1 = np.linalg.norm(t_coords - coords[P1], axis=1)
    h2 = np.linalg.norm(t_coords - coords[P2], axis=1)
    # get volume
    x = np.zeros(Np)
    np.add.at(x, P1, (Dp[:, 0] + Dt)/2*h1*w)
    np.add.at(x, P2, (Dp[:, 1] + Dt)/2*h2*w)
...
def effective(
    ...
    np.add.at(eff_vol, P1, factor*network[throat_volume])
    np.add.at(eff_vol, P2, factor*network[throat_volume])
    return eff_vol
