...
if zeta == 0:
    ...
    net["throat.zeta"] = jnp.where(mask, 1e-3, 1.0)
else:  
    # add zeta parameter
    mask = net["throat.micropore"]
...
def charge_solve(phi0, phi, dt, args, tol=1e-6, atol=0.0, maxiter=50):
    ...
    phi_new, info = jax.scipy.sparse.linalg.gmres(
        matvec,
        b_t,
        x0=phi,
        tol=tol,
...
for t in jnp.arange(t0+dt, tf+0.9*dt, dt):
    ...
    for g_iter in range(g_max_iter):
        ...
        p_max_iter = 10
        p_res = 100.0
        while p_res > 1e-9 and p < p_max_iter:
        ...
        c_new = jnp.clip(c_new, 1e-6, cf)  # 1e-6 used for zeta = 0.1
        c = w * c_new + (1 - w) * c_old
        net["pore.concentration"] = c
        # update charge conductance
        # start = time.time()
        # K = _update_charge_conductance(net, c + 1e-1)  # avoid zero conductivity
